"""XSD for Complex type extension output plugin"""
...
complex_type    = ('complex-types', 'complex-type')
instance        = ('complex-types', 'instance')
instance_list   = ('complex-types', 'instance-list')

def pyang_plugin_init():
    plugin.register_plugin(XSDCTPlugin())

class XSDCTPlugin(plugin.PyangPlugin):
...
def expand_locally_defined_defs(ctx, module, keyword, local_defs):
    """Create top-level typedefs for all locally defined typedefs."""
    for c in module.search(keyword):
        c.i_xsd_name = c.arg
    ...
    def add_def(obj):
        for t in obj.search(keyword):
            t.i_xsd_name = gen_name(t.arg, module.search(keyword) + \
                                        local_defs)
            local_defs.append(t)
        ...
        for c in obj.search('grouping'):
            add_def(c)
        for c in obj.search(('complex-types', 'complex-type')):
            add_def(c)
    for c in (module.i_children +
              module.search('augment') +
              module.search('grouping') +
              module.search(('complex-types', 'complex-type'))):
...
def emit_xsd(ctx, module, fd):
    ...
    expand_locally_defined_defs(ctx, module, 'typedef', module.i_local_typedefs)
    # first, create top-level complex types of local complex types
    expand_locally_defined_defs(ctx, module, complex_type, module.i_local_ctdefs)
...
def print_children(ctx, module, fd, children, indent, path,
    ...
    for c in children:
        ...
        if cn in ['container', 'list', 'leaf', 'leaf-list', 'anyxml',
                  'notification', 'rpc', instance, instance_list]:
            mino = ""
            maxo = ""
            atype = ""
            sgroup = ""
            ...
            if path == []:
                ...
                mino = ' minOccurs="0"'
            elif cn in ['list', 'leaf-list', instance_list]:
            ...
            elif cn in [instance, instance_list]:
                if hasattr(c.i_complex_type, 'i_xsd_name'):
                    atype = ' type="%s"' % c.i_complex_type.i_xsd_name
                else:
                    atype = ' type="%s:%s"' % (c.i_complex_type.i_module.i_prefix, c.i_complex_type.arg)
            ...
            for child in c.i_children:
                ...
                fd.write(indent + '    <xs:any minOccurs="0" '\
                               'maxOccurs="unbounded"\n')
                fd.write(indent + '            processContents="lax"/>\n')
                fd.write(indent + '  </xs:sequence>\n')
            # allow for augments
            fd.write(indent + '  <xs:any minOccurs="0"' \
                           ' maxOccurs="unbounded"\n')
...
def print_complex_type(ctx, module, fd, indent, ct):
    abstract_attr = ''
    if (ct.i_is_abstract):
        abstract_attr = ' abstract="true"'
    fd.write(indent + '<xs:complexType name="%s"%s>\n' % (ct.i_xsd_name, abstract_attr))
    print_description(fd, indent + '  ', ct.search_one('description'))
    plus_indent = ''
    if (ct.i_base_type is not None):
        base_ct_name = ct.i_base_type.i_module.i_prefix + ":" + ct.i_base_type.arg
        fd.write(indent + '  <xs:complexContent>\n')
        fd.write(indent + '    <xs:extension base="%s">\n' % base_ct_name)
        plus_indent = '    '

    fd.write(indent + plus_indent + '  <xs:sequence>\n')

    #print keys if hasattr(ct, 'i_key') and ct.i_key != []:

    # print the ct:type element
    mod_ct = ctx.get_module('complex-types')
    fd.write(indent + plus_indent + ('    <xs:group ref="%s"/>\n' % (mod_ct.i_prefix + ":" + 'type')))

    # print child elements
    for c in ct.i_children:
        if not hasattr(c, 'i_is_inherited'):
            print_children(ctx, module, fd, [c], indent + plus_indent + '    ', [ct.arg])

    # allow for augments 
    fd.write(indent + plus_indent + '    <xs:any minOccurs="0" maxOccurs="unbounded"\n')
    fd.write(indent + plus_indent + '        processContents="lax"/>\n')
